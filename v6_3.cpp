...
void checkNins(sets st,node* n,int l)
{
   ...
   for(i=0;i<st.rt_set_siz();i++)
   {
      ...
      for(j=0;j<tmp.size();j++)
      {
         ...
         {
            v[tmp1[j][k]][tmp1[j][k+1]]=1;
            //cout<<tmp1[j][k];
            v[tmp1[j][k+1]][tmp1[j][k]]=1;
            if(st.chk_bckup(tmp1[j][k],tmp1[j][k+1])==false)
            {
               cnt=1;
               break;
            }
         ...
         if(!cnt)//inserting spectrum in primary and sharing in backup path
         {
            ...
            for(k=0;k<tmp1[j].size()-1;k++)
            {
               //if(v[tmp1[j][k]][tmp1[j][k+1]]&&v[tmp1[j][k+1]][tmp1[j][k]])shrdIns(n,tmp1[j][k],tmp1[j][k+1],sp);
               //else n[tmp[j][k]].ins_spec(sp,b_ind,tmp[j][k+1],tmp[j][k]);
               n[tmp1[j][k]].ins_spec(b_ind[1],b_ind[0],tmp1[j][k+1],tmp1[j][k]);
            }
         }
      }
   }
   ...
   int l,i,x1,j;
      ...
      */
      ...
      while(f3>>a>>b>>c)//taking input
      {
         ...
         {
            cout<<v1[it]<<" ";
            if(v1.size()!=2&&it==v1.size()-1)break;
            e++;
            v[v1[it]]=1;
         }
         cout<<"\nweight "<<n[a].rt_wt(b)<<"\n";
